...
export const cases: CaseItem[] = [
  ...
  },
  {
    ...
    industries: ["testing-lab", "group-internal-lab"],
  ...
  },
  {
    ...
    industries: ["testing-lab", "regulated-lab", "research-lab"],
  ...
  },
  {
    ...
    industries: ["new-energy-lab", "manufacturing-lab"],
...
export function getCasesByIndustries(industrySlugs: string[] = [], excludeSlug?: string, limit = 2) {
  if (!industrySlugs.length) {
    return cases
      .filter((caseItem) => caseItem.slug !== excludeSlug)
      .slice(0, limit);
  }

  const matched = cases.filter(
    (caseItem) =>
      caseItem.slug !== excludeSlug &&
      caseItem.industries?.some((industry) => industrySlugs.includes(industry)),
  );
  ...
  const fallback = cases.filter(
    (caseItem) => caseItem.slug !== excludeSlug && !matched.some((item) => item.slug === caseItem.slug),
  );
