...
/*
 * 文件说明：该文件实现案例卡片组件。
 * 功能说明：用原本计划、发现风险和提前调整后的结果展示企业避免返工的真实路径。
 *
 * 结构概览：
 *   第一部分：CaseCard 组件
 */
...
export function CaseCard({ caseItem }: { caseItem: CaseItem }) {
  return (
    <article id={caseItem.slug} className="card scroll-mt-28">
      ...
      <p className="mt-3 text-copy">原本计划：{caseItem.problem}</p>
      <p className="mt-2 text-copy">发现风险后：{caseItem.action}</p>
      <p className="mt-2 text-copy">最终结果：{caseItem.result}</p>
      <div className="mt-4">
