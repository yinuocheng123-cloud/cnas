  ...
  return (
    <main className="min-h-screen bg-white text-ink">
      ...
      <HeroSection
        ...
        description={
          <>
            <p className="md:hidden">避免一上来就做材料，后面反复返工。</p>
      ...
      <section className="site-shell py-6 md:hidden">
        ...
        <div className="mt-4 grid gap-3">
          {mobilePainPoints.map((item) => (
            <div key={item} className="rounded-xl border border-amber-200/80 bg-amber-50/70 p-4 text-body font-medium leading-7 text-ink">
              {item}
            </div>
          ))}
        </div>
      ...
      <section className="site-shell pb-6 md:hidden">
        ...
        <div className="mt-4 grid grid-cols-2 gap-3">
          {mobileJudgmentItems.map(([title, summary]) => (
      ...
      <section className="site-shell pb-6 md:hidden">
        ...
        <div className="mt-4 grid items-start gap-4">
          {mobileFaqItems.map(([question, answer]) => (
            <div key={question} className="rounded-xl border border-line bg-white p-4">
              <h3 className="text-body font-semibold text-ink">{question}</h3>
              <p className="mt-2 text-copy">{answer}</p>
            </div>
          ))}
        </div>
      </section>
